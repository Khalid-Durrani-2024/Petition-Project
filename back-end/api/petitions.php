...
switch($request_method) {
    case 'GET':
        if (isset($_GET['sender'])) {
            $sender = $_GET['sender'];
            $stmt = $petitions->readBySender($sender);
        } else {
            $stmt = $petitions->read();
        }
        $num = $stmt->rowCount();
    ...
    case 'PUT':
        ...
        if (
            ...
            if ($petitions->update()) {
                ...
                http_response_code(503);
                echo json_encode(array("message" => "Unable to update Petition."));
            }
        } else {
            http_response_code(400);
    ...
    case 'DELETE':
        ...
        if ($petitions->delete()) {
            ...
            http_response_code(503);
            echo json_encode(array("message" => "Unable to delete petition some error occurred."));
        }
        break;
